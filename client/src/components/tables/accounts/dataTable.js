...
export default function DataTable() {
  ...
  const handleUpdateClick = (account) => {
    navigate('/edit-account', { state: { accountId: account._id } }); // Pass account ID via state
};
  ...
  const columns = [
    ...
    { field: 'tariff', headerName: 'Tariff', width: 130 },
    { field: 'loyalty', headerName: 'Loyalty', width: 130 },
    { field: 'addons', headerName: 'Addons', width: 130 },
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`${backLink}/accounts/all-accounts`, {
          headers: {
