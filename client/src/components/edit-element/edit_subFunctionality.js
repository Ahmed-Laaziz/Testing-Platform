...
export default function EditAssetPage () {
  const backLink = "http://localhost:5000";
    ...
    const [subFunctionality, setSubFunctionality] = React.useState(null);
    ...
    const handleChangeName = (event) => {
        ...
        subFunctionality.name = event.target.value;
    }
    const handleChangeDescription = (event) => {
        setDescription(event.target.value);
        subFunctionality.description = event.target.value;
    }
    

    console.log("SubFunctionality ID:", subFunctionalityId);
    ...
    React.useEffect(() => {
        const fetchSubFunctionality = async () => {
            try {
                ...
                const response = await axios.get(`${backLink}/functionalities/subFunctionality/${subFunctionalityId}`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
                setSubFunctionality(response.data); 
    ...
    const editSubFunctionality = async (event) => {
        ...
        try {
          ...
          await axios.put(`${backLink}/functionalities/subFunctionality/${subFunctionalityId}`, subFunctionality, {
            headers: { Authorization: `Bearer ${token}` },
          });
    ...
    return (
      ...
      <Alert onClose={handleClose} severity="success" variant="filled" sx={{ width: '100%' }}>
        ...
        <Grid size={6}>
      <TextField id="outlined-basic" label="Name" required variant="outlined" fullWidth value={name || subFunctionality?.name || ''} onChange={handleChangeName}/>
      </Grid>
      <Grid size={12}>
      <TextField
      fullWidth
            ...
            multiline
            rows={5}
            value={description || subFunctionality?.description || ''}
