...
export default function EditAssetPage() {
  ...
  const handleChangeName = (event) => {
    ...
    setFunctionality({ ...functionality, name: event.target.value });
  };

  const handleChangeDescription = (event) => {
    setDescription(event.target.value);
    setFunctionality({ ...functionality, description: event.target.value });
  };
  ...
  const handleProcessChange = (event, newValue) => {
    ...
    if (selectedProcess) {
      setProcessId(selectedProcess._id); // Store the process ID
      setFunctionality({ ...functionality, process: selectedProcess._id });
    }
  };
  ...
  useEffect(() => {
    const fetchFunctionality = async () => {
      try {
        ...
        setProcessId(response.data.process || ''); // Default process ID
        if (response.data.process) {
          fetchProcessById(response.data.process);
        }
      } catch (err) {
    ...
    const fetchProcesses = async () => {
      try {
        ...
        setAllProcesses(response.data); // Assuming this returns a list of processes with `_id` and `name`
      } catch (err) {
